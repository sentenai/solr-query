...
spec =
  describe "compile" $ do
    ...
    describe "range" $ do
      it "**" (test def ("foo" =: star `to` star) "q=foo:[* TO *]")

      describe "int" $ do
        it "[]" (test def ("foo" =: incl (int 5) `to` incl (int 6)) "q=foo:[5 TO 6]")
        it "{]" (test def ("foo" =: excl (int 5) `to` incl (int 6)) "q=foo:{5 TO 6]")
        it "[}" (test def ("foo" =: incl (int 5) `to` excl (int 6)) "q=foo:[5 TO 6}")
        it "{}" (test def ("foo" =: excl (int 5) `to` excl (int 6)) "q=foo:{5 TO 6}")
        it "*]" (test def ("foo" =: star `to` incl (int 5)) "q=foo:[* TO 5]")
        it "[*" (test def ("foo" =: incl (int 5) `to` star) "q=foo:[5 TO *]")
        it ">"  (test def ("foo" =: gt (int 5)) "q=foo:{5 TO *]")
        it ">=" (test def ("foo" =: gte (int 5)) "q=foo:[5 TO *]")
        it "<"  (test def ("foo" =: lt (int 5)) "q=foo:[* TO 5}")
        it "<"  (test def ("foo" =: lte (int 5)) "q=foo:[* TO 5]")

      describe "float" $ do
        it "[]" (test def ("foo" =: incl (float 5) `to` incl (float 6)) "q=foo:[5.0 TO 6.0]")
        it "{]" (test def ("foo" =: excl (float 5) `to` incl (float 6)) "q=foo:{5.0 TO 6.0]")
        it "[}" (test def ("foo" =: incl (float 5) `to` excl (float 6)) "q=foo:[5.0 TO 6.0}")
        it "{}" (test def ("foo" =: excl (float 5) `to` excl (float 6)) "q=foo:{5.0 TO 6.0}")
        it "*]" (test def ("foo" =: star `to` incl (float 5)) "q=foo:[* TO 5.0]")
        it "[*" (test def ("foo" =: incl (float 5) `to` star) "q=foo:[5.0 TO *]")
        it ">"  (test def ("foo" =: gt (float 5)) "q=foo:{5.0 TO *]")
        it ">=" (test def ("foo" =: gte (float 5)) "q=foo:[5.0 TO *]")
        it "<"  (test def ("foo" =: lt (float 5)) "q=foo:[* TO 5.0}")
        it "<"  (test def ("foo" =: lte (float 5)) "q=foo:[* TO 5.0]")

      describe "word" $ do
        it "[]" (test def ("foo" =: incl (word "a") `to` incl (word "b")) "q=foo:[a TO b]")
        it "{]" (test def ("foo" =: excl (word "a") `to` incl (word "b")) "q=foo:{a TO b]")
        it "[}" (test def ("foo" =: incl (word "a") `to` excl (word "b")) "q=foo:[a TO b}")
        it "{}" (test def ("foo" =: excl (word "a") `to` excl (word "b")) "q=foo:{a TO b}")
        it "*]" (test def ("foo" =: star `to` incl (word "a")) "q=foo:[* TO a]")
        it "[*" (test def ("foo" =: incl (word "a") `to` star) "q=foo:[a TO *]")
        it ">"  (test def ("foo" =: gt (word "a")) "q=foo:{a TO *]")
        it ">=" (test def ("foo" =: gte (word "a")) "q=foo:[a TO *]")
        it "<"  (test def ("foo" =: lt (word "a")) "q=foo:[* TO a}")
        it "<=" (test def ("foo" =: lte (word "a")) "q=foo:[* TO a]")

      describe "utctime" $ do
        it "[]" (test def ("foo" =: incl (datetime t1) `to` incl (datetime t2)) "q=foo:[\"2015-01-01T00:00:00Z\" TO \"2016-01-01T00:00:00Z\"]")
        it "{]" (test def ("foo" =: excl (datetime t1) `to` incl (datetime t2)) "q=foo:{\"2015-01-01T00:00:00Z\" TO \"2016-01-01T00:00:00Z\"]")
        it "[}" (test def ("foo" =: incl (datetime t1) `to` excl (datetime t2)) "q=foo:[\"2015-01-01T00:00:00Z\" TO \"2016-01-01T00:00:00Z\"}")
        it "{}" (test def ("foo" =: excl (datetime t1) `to` excl (datetime t2)) "q=foo:{\"2015-01-01T00:00:00Z\" TO \"2016-01-01T00:00:00Z\"}")
        it "*]" (test def ("foo" =: star `to` incl (datetime t1)) "q=foo:[* TO \"2015-01-01T00:00:00Z\"]")
        it "[*" (test def ("foo" =: incl (datetime t1) `to` star) "q=foo:[\"2015-01-01T00:00:00Z\" TO *]")
        it "**" (test def ("foo" =: star `to` star) "q=foo:[* TO *]")
        it ">"  (test def ("foo" =: gt (datetime t1)) "q=foo:{\"2015-01-01T00:00:00Z\" TO *]")
        it ">=" (test def ("foo" =: gte (datetime t1)) "q=foo:[\"2015-01-01T00:00:00Z\" TO *]")
        it "<"  (test def ("foo" =: lt (datetime t1)) "q=foo:[* TO \"2015-01-01T00:00:00Z\"}")
        it "<=" (test def ("foo" =: lte (datetime t1)) "q=foo:[* TO \"2015-01-01T00:00:00Z\"]")

    describe "boost" $ do
      it "word" (test def ("foo" =: word "bar" ^: 3) "q=foo:bar^3.0")
      it "phrase" (test def ("foo" =: phrase ["bar", "baz"] ^: 3) "q=foo:\"bar baz\"^3.0")

    it "should" (test def (should ("foo" =: word "bar")) "q=foo:bar")
    it "must" (test def (must ("foo" =: word "bar")) "q=+(foo:bar)")
    it "mustNot" (test def (mustNot ("foo" =: word "bar")) "q=-(foo:bar)")
    it "filt" (test def (filt ("foo" =: word "bar")) "q=#(foo:bar)")

    it "<>" (test def ("foo" =: word "bar" <> "bar" =: word "baz") "q=foo:bar bar:baz")
